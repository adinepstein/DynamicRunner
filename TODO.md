...
- [x] **0.3 — Finalize JSON schemas**
  ...
  - ✅ Dart codegen script/spec added: `shared/schemas/scripts/generate_dart_models.sh` + `shared/schemas/dart-codegen.spec.md` (targets `app/lib/src/generated/models` once app scaffold exists).
...
- [x] **0.4 — Agent prompt templates**
  - System prompt + few-shot examples for Planner; system prompt for Adapter; output schema enforcement.
  - **Drafted in [`backend/prompts/`](backend/prompts/) (`planner.system.v1.md`, `planner.fewshots.v1.md`, `adapter.system.v1.md`).** Added schema-valid examples in `backend/prompts/examples/` and validation script `backend/scripts/validate_prompt_examples.py` (+ pytest coverage in `backend/tests/test_prompt_example_outputs.py`).
...
- [~] **1.1 — Repos & monorepo layout**
  ...
  - ✅ Local monorepo layout now present (`app/`, `backend/`, `infra/`, `shared/`, `docs/`) with starter READMEs in `app/` and `infra/`.
  - Pending for full acceptance: GitHub branch protection on `main` (configured in GitHub UI after remote push).
- [x] **1.2 — Flutter scaffold**
  ...
  - ✅ `app/pubspec.yaml`, `lib/` entry (`main.dart`, Riverpod + `go_router` placeholder home), Hive + Supabase + Sentry + Firebase Messaging wired; `app/README.md` documents `flutter create .`, `--dart-define` for Supabase/Sentry, and `google-services.json` for FCM.
  - Acceptance: run `flutter create .` then `flutter pub get` / `flutter run` — placeholder home; tap Sentry test button with real `SENTRY_DSN`; Supabase placeholders OK until real keys via `--dart-define`.
- [x] **1.3 — FastAPI scaffold**
  - Project init with setuptools/pyproject, FastAPI, Pydantic v2, structlog, ruff + mypy, pytest, **Supabase JWT verifier middleware** (validates JWT signature against project JWKS; extracts `sub` as `uid`).
  - ✅ `backend/src/dynamicrunner/api/` — `GET /healthz`, `GET /me` (protected), JWKS-based middleware + pytest coverage in `backend/tests/test_api_auth.py`.
...
- [~] **1.4 — Supabase project + minimal Firebase (FCM only)**
  ...
  - **Setup guide:** [`docs/supabase-setup.md`](docs/supabase-setup.md) · templates: `backend/.env.example`, `app/dart_defines.json.example`.
  - ✅ Supabase project wired; Flutter + FastAPI JWT smoke test (`/me`) passing.
  - Pending: minimal **Firebase** project + `google-services.json` for FCM.
- [x] **1.5 — CI pipeline (GitHub Actions)**
  ...
  - ✅ [`.github/workflows/ci.yml`](.github/workflows/ci.yml) (backend + Flutter on PR/push). [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) (Render hook + Docker smoke on `main`). [`backend/Dockerfile`](backend/Dockerfile), [`infra/DEPLOY.md`](infra/DEPLOY.md), [`infra/render.yaml`](infra/render.yaml), [`app/dart_defines.ci.json`](app/dart_defines.ci.json).
  - **You:** add GitHub secret `RENDER_DEPLOY_HOOK` after creating Render service (see DEPLOY.md).
  - Acceptance: PR to `main` triggers pipeline green; hosted FastAPI URL serves new code within ~10 min of merge.
- [x] **1.6 — Database schema + RLS (Supabase)**
  ...
  - ✅ SQL migrations in [`supabase/migrations/`](supabase/migrations/) + [`supabase/README.md`](supabase/README.md). Applied to dev Supabase project.
...
- [x] **1.7 — Auth + profile row wired end-to-end**
  ...
  - ✅ Email sign-in/up UI (`SignInScreen`), router guard, `handle_new_user` trigger in migrations, home shows profile row. Sign-in working on phone.
  - Pending: Google sign-in; timezone picker on onboarding (Phase 3).
...
- [x] **2.2 — Encrypted credential storage**
  - Encrypt Garmin OAuth token blob with **`cryptography`** (Fernet) using **`APP_ENCRYPTION_KEY`** from env — store ciphertext in **Postgres** (`garmin_credentials` table). **`[future-aws]`**: envelope encryption with KMS + Secrets Manager.
  - ✅ `backend/src/dynamicrunner/crypto.py` (encrypt/decrypt/generate_key), `backend/src/dynamicrunner/garmin/__init__.py` (`GarminCredentialStore` + `GarminTokens`), 8 unit tests in `backend/tests/test_garmin_crypto.py` all passing.
  - Key rotation: generate new key → re-encrypt all rows → swap `APP_ENCRYPTION_KEY` in env → redeploy.
...
- [x] **2.3 — `POST /garmin/login` and `POST /garmin/mfa` endpoints**
  - Login flow per PRD Section 9; structured error responses for invalid credentials, locked account, MFA required.
  - ✅ `backend/src/dynamicrunner/garmin/client.py` — `login()` and `complete_mfa()` functions using `garth` + `curl_cffi`, with `LoginStatus` enum and `LoginResult`/`MfaResult` dataclasses.
